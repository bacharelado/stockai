...
def _usuario_por_sessao(request: Request, db: Session) -> models.Usuario:
    ...
    if not user_id or not empresa_id or session_version is None:
...
def forgot_password_page(request: Request):
    return HTMLResponse(
        """<!doctype html><html lang='pt-br'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Recuperar senha | StockAI</title><style>body{font-family:Segoe UI,Arial,sans-serif;background:#eef3fb;min-height:100vh;display:grid;place-items:center;margin:0;padding:20px}.card{width:min(420px,100%);background:#fff;padding:32px;border-radius:18px;box-shadow:0 20px 50px #17255420}h1{margin:0 0 10px;color:#172554}p{color:#64748b;line-height:1.5}label{display:block;color:#334155;font-weight:700;font-size:13px;margin-top:20px}input{width:100%;box-sizing:border-box;margin-top:7px;padding:12px;border:1px solid #cbd5e1;border-radius:10px;font:inherit}button{width:100%;margin-top:22px;padding:13px;border:0;border-radius:10px;background:#245f9f;color:#fff;font-weight:700;cursor:pointer}.back{display:block;text-align:center;margin-top:18px;color:#245f9f;text-decoration:none;font-size:13px}</style></head><body><main class='card'><h1>Recuperar senha</h1><p>Informe seu e-mail. Se existir uma conta associada, enviaremos instruções para redefinir a senha.</p><form method='post' action='/forgot-password'><label for='email'>E-mail<input id='email' name='email' type='email' autocomplete='email' required maxlength='254'></label><button type='submit'>Enviar instruções</button></form><a class='back' href='/login'>Voltar para o login</a></main></body></html>"""
    )
...
def forgot_password(request: Request, email: str = Form(...), db: Session = Depends(get_db)):
    ...
    usuario = (
        db.query(models.Usuario)
        .filter(models.Usuario.email == email_normalized, models.Usuario.ativo.is_(True))
        .first()
    )
    ...
    if usuario:
        ...
        registro = models.PasswordResetToken(
            ...
            expires_at=agora + RESET_TOKEN_TTL,
        )
        db.add(registro)
        db.commit()
        try:
            ...
            db.delete(registro)
            db.commit()
            raise HTTPException(status_code=503, detail="Não foi possível enviar o e-mail de recuperação. Tente novamente mais tarde.")

    return HTMLResponse(
        """<!doctype html><html lang='pt-br'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Verifique seu e-mail | StockAI</title><style>body{font-family:Segoe UI,Arial,sans-serif;background:#eef3fb;min-height:100vh;display:grid;place-items:center;margin:0;padding:20px}.card{width:min(460px,100%);background:#fff;padding:32px;border-radius:18px;box-shadow:0 20px 50px #17255420}h1{margin:0 0 10px;color:#172554}p{color:#64748b;line-height:1.5}.back{display:inline-block;margin-top:12px;color:#245f9f;text-decoration:none}</style></head><body><main class='card'><h1>Confira seu e-mail</h1><p>Se existir uma conta associada ao endereço informado, você receberá as instruções para redefinir a senha.</p><a class='back' href='/login'>Voltar para o login</a></main></body></html>"""
    )
...
def reset_password_page(token: str, db: Session = Depends(get_db)):
    registro = (
        ...
        .filter(
            models.PasswordResetToken.token_hash == _token_hash(token),
            models.PasswordResetToken.used_at.is_(None),
            models.PasswordResetToken.expires_at > _agora(),
        )
        .first()
    )
    ...
    return HTMLResponse(
        f"""<!doctype html><html lang='pt-br'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Nova senha | StockAI</title><style>body{{font-family:Segoe UI,Arial,sans-serif;background:#eef3fb;min-height:100vh;display:grid;place-items:center;margin:0;padding:20px}}.card{{width:min(420px,100%);background:#fff;padding:32px;border-radius:18px;box-shadow:0 20px 50px #17255420}}h1{{margin:0 0 10px;color:#172554}}p{{color:#64748b;line-height:1.5}}label{{display:block;color:#334155;font-weight:700;font-size:13px;margin-top:18px}}input{{width:100%;box-sizing:border-box;margin-top:7px;padding:12px;border:1px solid #cbd5e1;border-radius:10px;font:inherit}}button{{width:100%;margin-top:22px;padding:13px;border:0;border-radius:10px;background:#245f9f;color:#fff;font-weight:700;cursor:pointer}}</style></head><body><main class='card'><h1>Nova senha</h1><p>Escolha uma senha forte com pelo menos 12 caracteres.</p><form method='post' action='/reset-password'><input type='hidden' name='token' value='{token}'><label for='password'>Nova senha<input id='password' name='password' type='password' autocomplete='new-password' minlength='12' maxlength='256' required></label><label for='confirm_password'>Confirmar senha<input id='confirm_password' name='confirm_password' type='password' autocomplete='new-password' minlength='12' maxlength='256' required></label><button type='submit'>Redefinir senha</button></form></main></body></html>"""
    )
...
def reset_password(token: str = Form(...), password: str = Form(...), confirm_password: str = Form(...), db: Session = Depends(get_db)):
    ...
    db.query(models.PasswordResetToken).filter(
        models.PasswordResetToken.usuario_id == usuario.id,
        models.PasswordResetToken.id != registro.id,
        models.PasswordResetToken.used_at.is_(None),
    ).delete(synchronize_session=False)
...
def change_password_page(request: Request, db: Session = Depends(get_db)):
    ...
    return HTMLResponse(
        """<!doctype html><html lang='pt-br'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Alterar senha | StockAI</title><style>body{font-family:Segoe UI,Arial,sans-serif;background:#eef3fb;min-height:100vh;display:grid;place-items:center;margin:0;padding:20px}.card{width:min(420px,100%);background:#fff;padding:32px;border-radius:18px;box-shadow:0 20px 50px #17255420}h1{margin:0 0 10px;color:#172554}p{color:#64748b;line-height:1.5}label{display:block;color:#334155;font-weight:700;font-size:13px;margin-top:18px}input{width:100%;box-sizing:border-box;margin-top:7px;padding:12px;border:1px solid #cbd5e1;border-radius:10px;font:inherit}button{width:100%;margin-top:22px;padding:13px;border:0;border-radius:10px;background:#245f9f;color:#fff;font-weight:700;cursor:pointer}.back{display:block;margin-top:18px;text-align:center;color:#245f9f;text-decoration:none;font-size:13px}</style></head><body><main class='card'><h1>Alterar senha</h1><p>Conta: """ + usuario.username + """</p><form method='post' action='/change-password'><label for='current_password'>Senha atual<input id='current_password' name='current_password' type='password' autocomplete='current-password' required></label><label for='new_password'>Nova senha<input id='new_password' name='new_password' type='password' autocomplete='new-password' minlength='12' maxlength='256' required></label><label for='confirm_password'>Confirmar nova senha<input id='confirm_password' name='confirm_password' type='password' autocomplete='new-password' minlength='12' maxlength='256' required></label><button type='submit'>Alterar senha</button></form><a class='back' href='/dashboard'>Voltar ao painel</a></main></body></html>"""
    )
